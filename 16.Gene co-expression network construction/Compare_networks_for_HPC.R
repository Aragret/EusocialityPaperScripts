...
Networkfilelist = list.files("result_casteNormalisedAfterBC", 
                         pattern = paste0("*",spe,"*"),
                         full.names = T)
...
for (Networkfile in Networkfilelist){
  ...
  print(dim(Network))                                 # added for Bori and Bger
  Network = Network[Network$wTO != 0,]                # added for Bori and Bger
  print(dim(Network))                                 # added for Bori and Bger
...
csv_name=paste0("Codina_specific_normalised_afterBC/Group_table_normalisedAfterBC_",spe,".csv")
write.csv2(x = Group_table , file =  csv_name)
print(paste0(csv_name,"saved"))

print("saving Node groups...")
csv_name=paste0("Codina_specific_normalised_afterBC/Node_groups_normalisedAfterBC_",spe,".csv")
